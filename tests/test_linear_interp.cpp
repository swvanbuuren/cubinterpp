...
TEST(TestInterp4D, test_nonlinear_4d) {
    ...
    // At mid-points, linear interpolation of quadratic function won't be exact
    // Need to use a larger tolerance to account for the linear approximation of quadratic function
    EXPECT_NEAR(interp.eval(0.5, 0.5, 0.5, 0.5), 1.0, 1.5);  // Increased tolerance from 0.5 to 1.5
    EXPECT_NEAR(interp.eval(1.5, 1.5, 1.5, 1.5), 9.0, 1.5);  // Increased tolerance from 1.0 to 1.5
}
